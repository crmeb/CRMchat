                ...
                <div>
                    ...
                    <div class="ivu-pt-8" style="height: 42px;">
                        <span style="display: inline-block" class="ivu-mr">
                          ...
                          日环比 {{Number(item.today_ratio)}}% <Icon :type="Number(item.today_ratio)>=0?'md-arrow-dropup':'md-arrow-dropdown'" class="iconColor" :class="Number(item.today_ratio)>=0?' ':'on'"/>
                        </span>
                        <span style="display: inline-block">
                            <!--周同比 <Trend :flag="Number(item.week_ratio)>=0?'up':'down'">{{Number(item.week_ratio)}}%</Trend>-->
                           周环比 {{Number(item.week_ratio)}}% <Icon :type="Number(item.week_ratio)>=0?'md-arrow-dropup':'md-arrow-dropdown'" class="iconColor" :class="Number(item.week_ratio)>=0?' ':'on'"/>
                        </span>
                    </div>
