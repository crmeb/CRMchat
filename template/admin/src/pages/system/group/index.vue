<template>
  <div>
    ...
    <Card :bordered="false" dis-hover class="ivu-mt">
      <Form ref="formValidate" :model="formValidate" :label-width="labelWidth" :label-position="labelPosition" @submit.native.prevent>
        <Row type="flex" :gutter="24">
          ...
          <FormItem label="数据搜索：" label-for="status2">
            <Input search enter-button placeholder="请输入ID,KEY,数据组名称,简介" v-model="formValidate.title" @on-search="userSearchs" />
          </FormItem>
          </Col>
        </Row>
        <Row type="flex">
      ...
      <Table :columns="columns1" :data="tabList" ref="table" class="mt25" :loading="loading" highlight-row no-userFrom-text="暂无数据" no-filtered-userFrom-text="暂无筛选结果">
...
export default {
  ...
  methods: {
    // 跳转到组合数据列表页面
    goList(row) {
      this.$router.push({ path: '/admin/system/config/system_group/list/' + row.id })
    },
    // 列表
    getList() {
      this.loading = true
