...
<script>
    ...
    import { userGroupApi, groupAddApi, groupSaveApi } from '@/api/user'
    export default {
        name: 'user_group',
        data () {
            return {
        ...
        methods: {
            // 添加
            add () {
                this.$modalForm(groupAddApi()).then(() => this.getList())
            },
            // 分组列表
            getList () {
                this.loading = true
            ...
            // 修改
            edit (id) {
                this.$modalForm(groupSaveApi(id)).then(() => this.getList())
            },
            // 删除
            del (row, tit, num) {
                let delfromData = {
